...
**Status:** proposal · ready for review · no implementation started ·
**Executes:** offline viewing requested 2026-08-05 · **Written:** 2026-08-05
against `83be7dc481` on `codex/fix-apple-subtitle-warning`
...
4. The device downloads the package in the background. The UI says
   **Downloading** with percent and bytes.
...
turning on airplane mode, and stopping `plurxd`, a downloaded two-hour title
...
Changing servers or signing in as another user hides, but does not silently
...
until that same server/user profile is active again.
...
| Bitmap/styled subtitle | If selected, burn it into the video and label the package **Subtitles included in picture**. |
| Container | Finished fMP4 HLS with `#EXT-X-PLAYLIST-TYPE:VOD` and `#EXT-X-ENDLIST`. |
...
reuses the pre-transcode cache's content-addressed HLS rather than introducing a
...
- **No PGS object bundle.** A selected bitmap/styled subtitle is burned. The
  online `pgs-v1` overlay is many authenticated objects and is not yet an
...
`OfflineManager` owns a serial, priority-aware preparation queue. One package
may encode per node at a time. A user-requested offline package is below live
playback and above the speculative scheduled pre-transcode producer. Both
background paths share the same one-producer permit so they cannot mistake one
another's resumable claim for abandoned work.

Live playback preempts offline preparation through the existing admission
mechanism. Preemption terminates at a published segment boundary, releases the
encoder slot, and leaves the claim plus staged parts to resume. A download may
take longer while somebody watches a film; it must never make that film stall.
...
- The video cache location is complete and contains a VOD playlist ending in
  `#EXT-X-ENDLIST`.
- `init.mp4` and every named media segment exist with their final names.
...
- The package's actual byte count is stored for the client and cache budget.
...
- `estimated_bytes = duration_seconds × total_kbps × 1000 / 8 × 1.05`.
  The 5% is mux/playlist headroom, not a storage guarantee.
...
- `offline_mode` is `native` for simple WebVTT-convertible text and `burned`
  for bitmap or styled text. Unknown selections are rejected, not ignored.
...
cache ceiling cannot hold the estimated package plus currently pinned bytes.
`cache.max_gb = 0` returns this explicit error; it must not start hours of work
that housekeeping immediately deletes.
...
· encoder_failed · subtitle_failed · package_expired`. Internal ffmpeg paths
and stderr do not cross the API.
...
### 5.4 `POST /api/v1/offline/packages/{id}/leases`

Only a ready package can mint a transfer lease. Response (`201 Created`):
...
The path contains a new random 256-bit secret. Store only its SHA-256 hash.
The lease grants read access to this package and nothing else, lasts seven
days, survives a server restart, and pins the matching cache entry against LRU
eviction. Multiple leases may exist so a client that crashed before saving the
first response can mint another without invalidating an active background
task. Neither the secret nor full manifest URL is logged.
...
GET /api/v1/offline/media/{secret}/init.mp4
GET /api/v1/offline/media/{secret}/{segment}.m4s
GET /api/v1/offline/media/{secret}/subs/{index}/index.m3u8
GET /api/v1/offline/media/{secret}/subs/{index}/{segment}.vtt
```

Every request hashes the secret, verifies the unexpired lease and complete
...
removes this package's leases and pin, but leaves the content-addressed VOD in
...
`recipe_hash` is nullable only between validated insertion and recipe
construction. It deliberately does not reference the recipe table: the hash is
known before `claim_cache_entry` creates that row, and a failed job must retain
its diagnostic even if its failed claim is removed.
...
lease create/hash lookup/touch · complete/cancel · expired sweep · active
...
) -> Result<Produced, OfflineProduceError>;
```

`ensure_offline` always builds a zero-offset `SessionKind::Transcode` with the
selected audio, a burn only for `Burn`, and the current tone-map preference.
It calls the same `options_for`, recipe hash, claim, resumable `produce_into`,
atomic publication, and cache completion used today. A native subtitle does
not change video bytes, so it is not added to the transcode recipe; the package
row records it and readiness separately gates on its extracted VTT.

Refactor today's scheduled `produce` over the same primitive. Do not copy its
recipe construction. The cache already learned once that a producer and a
viewer choosing tracks through different paths creates an expensive cache that
never hits.

### 7.2 Queue and resource rules

- One background producer per node across scheduled and requested work.
...
- A live waiter preempts at the next complete segment and receives the slot
  inside the existing bounded admission window.
...
- At most 100 nonfailed package rows per user. The 101st returns `429` with a
  useful message; this bounds an authenticated accidental tap loop without
  making a normal episode queue small.

### 7.3 Cache budget and pins

Extend `cachekeep::sweep` to exclude recipe hashes referenced by an unexpired
ready/preparing package or lease. Pinned bytes still count toward
`cache.max_gb`; the sweep logs `pinned_bytes` and says when pins prevent it from
meeting the ceiling. Do not report an under-budget fiction.
...
This also closes a latent playback-cache race: an ordinary cached VOD session
should expose its active recipe hash to the same protected set while segments
are being served, so housekeeping cannot evict a directory beneath a viewer.

### 7.4 Offline HLS is a separate presentation wrapper

The cached `index.m3u8`, init, and media segments are reused without copying.
...
#EXT-X-STREAM-INF:BANDWIDTH=<rung peak>,AVERAGE-BANDWIDTH=<rung total>,
  RESOLUTION=<scaled output>,CODECS="avc1.640034,mp4a.40.2",
  CLOSED-CAPTIONS=NONE[,SUBTITLES="subs"]
index.m3u8
```

The actual line is unwrapped. `VIDEO-RANGE` is absent because every v1 package
is SDR. Native subtitle playlists mirror the completed video timeline and end
with `#EXT-X-ENDLIST`. A selected burned subtitle produces no subtitle group.
...
iOS 17; compile against the pinned Xcode SDK and use
`AVAssetDownloadConfiguration` where available without raising the deployment
target. The tvOS target must continue compiling in Debug and Release.
...
`#if os(iOS)`; the four Apple build/test configurations remain the gate.
...
crash. The local AVFoundation destination URL/bookmark from the delegate is the
playback identity across launches; do not reconstruct a path by convention.
...
Create one background asset session with a stable bundle-qualified identifier.
Restore its tasks on launch and map `taskDescription` to the local item id. Use
the system task's `Progress` for the device-transfer percentage. The App
delegate/SwiftUI lifecycle must retain and call the background-session
completion handler after delegate events are drained.

Wi-Fi-only maps to the background configuration's cellular/expensive-network
policy. A user switching to Any Network applies to newly created tasks; the UI
states when an existing task is waiting for Wi-Fi. Do not emulate background
work with an ordinary `Task` that dies when iOS suspends the app.

Save the delegate-provided local URL immediately. On completion, load
`AVURLAsset.isPlayable`, verify a nonzero duration, save the artwork/index, mark
Downloaded, then best-effort acknowledge the server package. If validation
fails, remove the corrupt package and show Retry.
...
URL. It keeps the same PlayerSurface, Now Playing metadata, seek, skip markers
saved in the snapshot, native subtitle selection, and Picture in Picture where
the local presentation supports it. Quality is fixed and the info panel says
...
Add Downloads to the iOS `TabView`. If bootstrap cannot reach the server but a
saved profile has downloads, RootView enters an offline-capable shell
immediately; it does not strand the user on `ReconnectView`. Home can retain a
...
Use the already pinned Media3 1.10.1 family. Add only the Media3 database and
optional scheduler artifact required by the chosen implementation; keep every
Media3 artifact on the same version.

The manifest adds `FOREGROUND_SERVICE`,
`FOREGROUND_SERVICE_DATA_SYNC`, the unexported service with
`foregroundServiceType="dataSync"`, and the Media3 restart intent. Android 13+
asks for notification permission at the moment the first background download
needs it, with a clear fallback if denied. A hidden notification is not
permission to let the process die silently.
...
the content URI and the MIME hint is HLS. Empty `streamKeys` is acceptable
because the offline master deliberately contains one video/audio variant and
at most one subtitle rendition.
...
and transfer state. The catalog reconciles orphaned intent, failed requests,
...
with the shared download cache and **no upstream fallback**. A cache miss must
...
The offline controller bypasses `decision`, HLS creation, telemetry polling,
session deletion, and online next-episode lookup. It retains the same controls,
PiP, local track selection, and progress cadence. Autoplay may continue only
...
`currentFormFactor().television` is true. Phone/tablet Home adds a Downloads
...
Version 1 reuses `POST /items/{id}/progress`; the final local update is sent as
a fresh progress mutation when connectivity returns, so the flight's most
recent local viewing wins at that moment. Send at most once per item, newest
only, after saved-session validation succeeds. Clear only on 2xx; 401/403 waits
for reauthentication; transport/5xx backs off without losing the row.

This is intentionally a final-state outbox, not an event history. It avoids a
hundred ten-second beats flooding the server and Trakt after a flight. A future
multi-device conflict protocol may add a watch-state revision; it is not needed
to make one device's disconnected session durable.
...
local player creates the local progress record. On reconnection the existing
server-side watch/Trakt path runs once from the applied final state.
...
- A lease is 256 random bits, stored only as SHA-256, scoped to one completed
  recipe/track selection, read-only, and seven days maximum.
- The HTTP trace layer must replace `/offline/media/{secret}/...` with a
  constant redacted route before it creates a span. Default URI logging would
  otherwise leak the credential even if the handler never mentions it.
- Logs include package id, file id, user id, phase, recipe prefix, and byte
  ...
  client path.
- Typed resource routes accept only exact init/segment/subtitle names. No
  arbitrary relative path is joined to the cache root.
- Expired/cancelled/complete packages revoke every outstanding server lease.
...
- Changing a server address during an active transfer may require Retry and a
  new lease URL. It cannot corrupt a completed local package.
...
| Lease expires before completion | Failed · Connection authorization expired | Mint a new lease and restart transfer; keep any platform-safe partial cache, otherwise start clean. |
| App is killed | No state loss | Platform manager continues/restores; local intent maps callbacks after relaunch. |
...
2. Download it with `AVAssetDownloadURLSession` on a physical iPhone/iPad,
   suspend and kill the app mid-transfer, relaunch, and finish.
...
Do not land a throwaway launch-argument credential harness in Release source;
the Apple README's fencing rule remains in force.
...
this document. If Apple rejects the current fMP4 VOD, stop here and correct the
...
lease hashing/ownership, expiry cleanup, and the typed empty media routes. Add
the shared native API fixture fields and a new `offline.viewing` functionality
point to [`validation/points.toml`](../validation/points.toml), depending on
`playback.pipeline · server.api · identity.access · apple.client ·
android.client` with the appropriate server/client checks.
...
read a byte; `make check` passes.

### 14.3 M2 — resumable portable package production

Extract `ensure_offline`, share recipe construction with scheduled production,
add the serial priority queue, progress publication, cache pins/budget checks,
complete offline master/subtitle serving, restart recovery, and activity/
metrics. Ensure preparation never enters playback/Trakt/watch paths.

**Acceptance:** prepare the synthetic HEVC/HDR + TrueHD + PGS case as 720p
H.264/AAC/SDR with the PGS visibly burned; prepare an SRT case with a selectable
...
**Acceptance:** all iOS and tvOS Debug tests and Release builds pass; a physical
iPhone downloads in the background across suspension and app termination;
...
**Acceptance:** JVM/lint and phone instrumentation pass; API 23 and current API
...
Run the §15 matrix, fix the failures it finds, advance both store build numbers,
and update [README.md](../README.md), [FEATURES.md](FEATURES.md),
...
**Acceptance:** `make validate-full` is green on macOS with both native
toolchains and the physical-device evidence table is complete. A release
...
| Store/API integration | Migration, idempotency, ownership, leases, expiry, cache pins, HTTP media shapes | Every endpoint has success, unauthenticated, wrong-owner, invalid-input, and retry coverage where applicable. |
...
- Create idempotency and conflict, row cap, ownership on every mutation.
...
- VOD completeness, actual output master values, immutable typed resources,
...
| Ready + lease | One platform task with stable local id and correct metadata. |
...
| Offline play | Remote decision/session/status/progress clients are not invoked. |
...
| App process killed mid-transfer | Required | Required |
...
4. **A durable package row plus hashed, expiring media lease.** Background media
   engines need headerless child fetches and server restarts must not break the
   URL. The cost is migration v14 and expiry housekeeping.
...
8. **Offline progress is a durable final-state outbox.** It preserves the
   disconnected session without replaying hundreds of beats. The newest local
   final state wins when it reconnects; richer cross-device conflict resolution
   is deferred.
...
downloads, and desktop/TV surfaces only after production telemetry answers:
preparation latency · requested quality mix · package size · cancellation rate
· cache-pin pressure · background failure rate · how often users redownload for
another language. Until those numbers exist, the narrow contract is the safer
one to keep.

## 17. Platform references — primary documentation
