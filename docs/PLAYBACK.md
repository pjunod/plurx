...
| `apple.hls-buffer-window` | Growing HLS forward buffer | AVPlayer prefers a 60 s buffer on live copy/transcode sessions, but a physical iPad fetched about 120 s ahead; the preference is not a cap. The server therefore retains 180 s behind the download frontier: the measured 120 s lead plus 30 s of back buffer and 30 s for retry/reload. Direct files and completed cached HLS keep AVPlayer's default. | Rust physical-iPad frontier regression + XCTest item-configuration guard |
...
- **The ahead-window suspend.** Once a session is more than
  ...
  30 s below that ceiling (150 s under the default). The release point must
  stay above AVPlayer's measured no-fetch waterline: on the physical iPad,
  AVPlayer stopped fetching with about 95 s buffered and left the producer
  138 s ahead, so the old half-window release at 90 s was unreachable — only
  another fetch could move the server's frontier. Byte and global disk limits
  still release at half because those are hard capacity bounds. This is the
  bound `-re` used to provide, minus the part where `-re` also capped the
  buffer. A stopped process costs nothing, resumes instantly, and — unlike a
  rate limit — adapts to a viewer who pauses. SIGKILL works on a stopped
  process, so the idle reaper and the admin stop button need no special case.
...
- **HLS session disk.** A live HLS writer's history grows for its whole life,
  ...
  `hls_ahead_max_secs + 180 s` of content whatever the encoder's speed. Once
  the reaper removes an older prefix, the client
