...
| `server.dolby-vision` | Preserve vs strip vs re-encode DV | A client-approved profile is preserved. An unsupported profile with a compatible base and `dovi_rpu` becomes a strip remux. Without both, re-encode. Apple-supported DV profiles still request a normalized copy-HLS envelope. A Profile 5 compatibility transcode stamps its full-range BT.2020/PQ input facts before scale/tone-map so hardware decode cannot leave zscale with an unspecified colorspace. | DV profile matrix in `playback/mod.rs`; Profile 5 filter-order regression in `transcode/mod.rs` |
