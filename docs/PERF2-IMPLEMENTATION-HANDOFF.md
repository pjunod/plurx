...
the N1 peak/storage contracts on 2026-08-12, the N4.2 storage contract on
2026-08-14, and the append-only reconciliation after reading state shipped on
2026-08-20. D1–D6 remain runtime settings; D7–D10 are binding implementation
...
| D10 append-only reconciliation | preserve shipped reading state at SQLite v20, first-class book metadata at SQLite v21, and replicated v6; credential-generation correction at SQLite v22; N3 SQLite v23; protocol v4 unchanged |

If the operator changes D1–D6 mid-build, that lands as a settings-default
commit with a CHANGELOG line. Changing D7, D8, D9, or D10 is a new contract decision
...
replicated schema v5 with protocol v4. D10 preserves the shipped reading-state
migration at SQLite v20, first-class book metadata at SQLite v21, and
replicated v6, applies the N4.2 credential-generation correction at SQLite
v22, and moves N3 to SQLite v23.
...
Shaping harness for playback-lab first — **implemented and accepted by
#143**: `--network-profile
...
so tuning them is a reviewable manifest change and never a script edit. Its
closure evidence is recorded on #143 and PR #145: a reproducible shaped
`outcome: recovery` baseline, byte-identical normalized base-versus-candidate
unshaped traces, and a green required gate — not claims inferred from source.
Then the web controller
