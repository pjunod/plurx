...
| Books and ebooks | In-app EPUB plus external PDF handoff | iPhone/iPad read EPUB online/offline and PDF online; PDFKit uses exact-revision temporary bytes, page locators, local search, and explicit completion. tvOS exposes no text reader. | Physical VoiceOver and representative PDF/device acceptance; PDF offline remains external handoff |
...
| Playback info | Mini, Standard, and Debug views with detailed source, output, network, encoder, and stall data | Mini keeps the picture visible; Standard retains the ten-foot tvOS Source/Playing/Server dashboard; Debug adds the build, transport, player/access-log state, and live server-session fields. Selected-subtitle route/state remains explicit (`PGS overlay · preparing`, ready overlay, unavailable, native WebVTT, or burned in). Sustained and self-recovered stalls reach the bounded server log with HLS session and per-item attempt identity, contiguous loaded runway, AVPlayer wait/buffer flags, access-log request/transfer counters, and the aged last successful server supply snapshot; live ingest replaces it with fresher session state when possible. Recovery TTFF is a separate attempt. | Add frame presentation rate where AVPlayer exposes it; validate correlated stall ingest on physical iPad |
...
| PiP/AirPlay | Browser/platform dependent | Explicit iOS PiP control on the AVPlayer surface; source coverage proves unavailable AVKit and in-app-only PGS states remain tappable and explain themselves instead of becoming inert. The not-ready explanation clears after five seconds instead of pinning the player banner or iOS system chrome; an AVKit start failure remains visible. A physical non-PGS H.264/AAC baseline on iPhone Air (iOS 26.6) and iPad Pro 13-inch (M4, iPadOS 26.6) recorded `isSupported=true`, an attached controller, `isPossible=true`, the tap and start request, `isActive=true`, and clean return or stop. While a PGS application overlay is active, PiP and external playback are refused with a visible explanation because those outputs do not carry the sibling overlay layer, and the app never substitutes an SDR burn. | Confirm the documented PGS refusal on iPad Pro; add a dedicated AirPlay affordance and remote-device session tests for non-overlay playback |
