...
> Status (2026-08-20): source is v0.2.7, Apple build 69. Native text
...
> landed. Build 69 marks a duration-estimated Skip Credits button as an
> estimate, so it no longer reads exactly like a chapter-derived one. Build 68
> adds revision-bound ebook reading-state wire models and
> authenticated API routes without exposing a tvOS reader. Build 67 adopts the
...
> resolved status. Repository evidence still says build 68 has not reached
