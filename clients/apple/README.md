...
> same regression suite. Build 69 tells a viewer when a Skip Credits button
> came from a duration estimate rather than a real chapter, so a guess no
> longer reads as an exact marker. Build 68 adds the revision-bound ebook reading-state
