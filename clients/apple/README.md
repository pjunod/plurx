...
> same regression suite. Build 49 resets a stalled growing-HLS item against
> its existing session first and skips copy-keyframe preroll if a new session
> is required, so recovery preserves the exact film position. Build 46 adds a
> one-shot, session-joined TTFF beacon
